...
{
    base->CHANNEL[channel].LDVAL = ticks;
}
...
{
    base->CHANNEL[channel].TCTRL &= ~PIT_TCTRL_CHN(0);
}
...
{
    base->CHANNEL[channel].TCTRL &= ~PIT_TCTRL_TIE(0);
}
...
{
    base->CHANNEL[channel].TCTRL &= ~PIT_TCTRL_TEN(0);
}
...
static inline void PIT_Config(PIT_Type *base, ePIT_Chnl_Type channel, uint32_t ticks)
{
    ...
    PIT_SetTimerPeriod(base, channel, ticks - 1U);
}
